...
class SimulationConfig:
    ...
    weather_task_rate_multiplier: Dict[str, float] = field(default_factory=lambda: {
        # More adverse weather usually generates additional verification demand.
        "Clear": 1.00,
        "Rain": 1.08,
        "Fog": 1.12,
    })
    """
    Multiplier applied to task arrivals by current weather.
    This makes workload intensity condition-dependent (more realistic operations).
    """
    ...
    lighting_task_rate_multiplier: Dict[str, float] = field(default_factory=lambda: {
        # Lower-visibility periods tend to produce more safety-relevant tasks.
        "Day": 1.00,
        "Dusk": 1.05,
        "Night": 1.10,
    })
    """
    Multiplier applied to task arrivals by current lighting state.
    Combined with weather multiplier, this yields scenario-driven workload levels.
    """
