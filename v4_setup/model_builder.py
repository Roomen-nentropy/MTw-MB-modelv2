...
def build_skill_based_pyvrp_model(
    ...
    *,
    ...
    Compatibility is encoded by route penalties using profile-specific
    edge distances: for a given vehicle tier/profile, edges that enter
    (and leave) incompatible clients are assigned a prohibitive distance.

    This implements the thesis idea of "compatibility penalty logic"
    (Phase 3) in a modeling-friendly way.
    """
    ...
    for tier, profile in zip(vehicle_tiers, profiles):
        ...
        fixed_cost_signal = max(0.0, (tier.cost_multiplier ** vehicle_fixed_cost_exponent) - 1.0)
        fixed_cost = int(round(fixed_cost_signal * vehicle_fixed_cost_scale * max(0.0, operating_cost_multiplier)))
    ...
    # Compatibility penalties via profile-specific edges.
    # For profile p with Skill=Skill_k: client j incompatible if Skill_k < Req_j.
    for profile_idx, (tier, profile) in enumerate(zip(vehicle_tiers, profiles)):
        ...
        for i, loc_i in enumerate(loc_objs):
            for j, loc_j in enumerate(loc_objs):
                ...
                # Penalize entering incompatible clients.
                if loc_is_client[j]:
                    ...
                    if skill_k < req_j:
                        ...
                        # Realism: overqualified assignment on simple tickets
                        # carries extra operating overhead.
                        skill_gap = max(0, skill_k - req_j)
                        if skill_gap > 0:
                            dist = int(round(dist * (1.0 + skill_gap * overqualification_cost_penalty_per_level)))

                # Penalize leaving incompatible clients as well.
                if loc_is_client[i]:
