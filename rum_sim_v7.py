...
rum_sim_v7.py
-------------
Real-world map + next-day forecast simulation using the v7 package.

V7 objective:
  - Keep a larger owned fleet (2x baseline),
  - deploy only an optimizer-selected active subset for tomorrow's forecast,
  - derive road domains from full OSM highway tags when .osm.xz is provided.
"""
...
from v7.bbbike_map_sampler import suggest_sindelfingen_depot_xy
from v7.config import DepotConfig, FleetTierConfig, SimulationConfig
from v7.deployment import choose_optimal_deployment, scale_owned_fleet
from v7.open_meteo_weather import fetch_save_default_stuttgart
...
def build_config() -> SimulationConfig:
    ...
    base_fleet = [
        ...
        FleetTierConfig("Advanced", skill_k=2, cost_multiplier=1.35, num_available=1),
    ]
