...
def run_simulation(config: SimulationConfig) -> SimulationSummary:
    ...
    # Why: real Open-Meteo series replaces synthetic weather + lighting when configured.
    open_meteo_driver = None
    if getattr(config, "open_meteo_hourly_csv", None):
        from .open_meteo_weather import OpenMeteoSeriesDriver

        open_meteo_driver = OpenMeteoSeriesDriver.from_csv(config.open_meteo_hourly_csv)
        if config.verbose and config.total_periods > len(open_meteo_driver):
            print(
                f"  [open-meteo] Warning: total_periods={config.total_periods} exceeds "
                f"hourly rows={len(open_meteo_driver)}; indices wrap modulo length."
            )
        weather_sim = None
        lighting_sim = None
    else:
    ...
    for t in range(config.total_periods):
        ...
        if open_meteo_driver is not None:
            weather = open_meteo_driver.weather_at(t)
            lighting = open_meteo_driver.lighting_at(t)
        else:
            weather = config.initial_weather if t == 0 else weather_sim.step()
...
def _print_banner(config: SimulationConfig) -> None:
    ...
    if getattr(config, "open_meteo_hourly_csv", None):
        print(f"  Open-Meteo CSV  : {config.open_meteo_hourly_csv}")
    if getattr(config, "bbbike_xz_path", None):
        print(f"  BBBike map      : {config.bbbike_xz_path}")
    print("=" * 66)
